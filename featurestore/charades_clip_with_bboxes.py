...
def extract_relationship_features(conn, config, sequence_length, batch_size, num_threads, dataset="charades", patch_size=(224, 224)):
    ...
    feature_file_dir = os.path.join(config["db_dir"], "features", "charades_clip_with_bboxes", "relationship")
    ...
    for batch in tqdm(video_iterator):
        ...
        # Run CLIP model
        images = []
        for patch, boxes in zip(image_patches, batch_boxes):
            boxes = torch.tensor(boxes).reshape(2, 4).to(device)
            patch = draw_bounding_boxes(patch.to(dtype=torch.uint8), boxes, colors=['red', 'blue'])
            images.append(patch)
        images = torch.stack(images, dim=0).to(device)
        # show_sequence(images, parquet_vids, parquet_fids, parquet_o1_oids, parquet_o2_oids)
        time2 = time.time()
        ...
        with torch.no_grad():
            features = clip_model.get_image_features(pixel_values=inputs) # torch.FloatTensor of shape (N, 512)
        time3 = time.time()
