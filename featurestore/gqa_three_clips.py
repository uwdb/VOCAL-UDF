...
def extract_attribute_features(conn, config, batch_size, num_threads, dataset="gqa", patch_size=(224, 224)):
    ...
    vids = conn.execute("SELECT DISTINCT vid FROM gqa_metadata").df()["vid"].tolist()
    image_files = [
        os.path.join(
            image_directory,
            f"{vid % 10}",
...
def data_loader(conn, batch_size, patch_size, device, dataset="gqa"):
    # Only consider top-50 object classes
    object_domain = ['window', 'man', 'shirt', 'tree', 'wall', 'person', 'building', 'ground', 'sky', 'sign', 'head', 'pole', 'hand', 'grass', 'hair', 'leg', 'car', 'woman', 'leaves', 'trees', 'table', 'ear', 'pants', 'people', 'eye', 'water', 'door', 'fence', 'nose', 'wheel', 'chair', 'floor', 'arm', 'jacket', 'hat', 'shoe', 'tail', 'clouds', 'leaf', 'face', 'letter', 'plate', 'number', 'windows', 'shorts', 'road', 'flower', 'sidewalk', 'bag', 'helmet']
    obj_parameters = ','.join('?' for _ in object_domain)
    df_grouped = conn.execute(f"""
    ...
    """, object_domain + object_domain).df().groupby(['vid', 'fid'])
...
def extract_relationship_features(conn, config, batch_size, num_threads, dataset="gqa", patch_size=(224, 224)):
    clip_model_name = os.path.join(config['model_dir'], "clip-vit-base-patch32")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    clip_model = CLIPModel.from_pretrained(clip_model_name).to(device)
