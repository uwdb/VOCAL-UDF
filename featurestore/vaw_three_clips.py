...
from transformers import CLIPProcessor, CLIPModel, AlignModel
...
def _compute_new_box_after_crop(row, image_size, crop_size):
    ...
    h_ratio = crop_size[0] * 1.0 / (max(o1_y2, o2_y2) - y_offset)
    w_ratio = crop_size[1] * 1.0 / (max(o1_x2, o2_x2) - x_offset)
...
def extract_attribute_features(conn, config, feature_extractor, batch_size, num_threads, dataset="vaw"):
...
def data_loader(conn, batch_size, patch_size, device, dataset="vaw"):
    df_grouped = conn.execute(f"""
    ...
    for vid in tqdm(vids):
        if (vid, 1) not in df_grouped.groups:
            continue
        res = df_grouped.get_group((vid, 1))
        frame = get_frame(vid)
        _H, _W, _C = frame.shape
        # NOTE: Due to data noise, multiple objects can have the same oid
        for _, row in res.iterrows():
            ...
            if 0 <= roi_x1 < roi_x2 and 0 <= roi_y1 < roi_y2:
                ...
                new_o1x1, new_o1y1, new_o1x2, new_o1y2, new_o2x1, new_o2y1, new_o2x2, new_o2y2 = _compute_new_box_after_crop(row, (_H, _W), crop_size=patch_size)
...
def extract_relationship_features(conn, config, feature_extractor, batch_size, num_threads, dataset="vaw"):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    if feature_extractor == "clip":
        clip_model_name = os.path.join(config['model_dir'], "clip-vit-base-patch32")
        clip_model = CLIPModel.from_pretrained(clip_model_name).to(device)
    ...
    feature_file_dir = os.path.join(config["db_dir"], "features", f"{dataset}_three_{feature_extractor}s", "relationship")
    os.makedirs(feature_file_dir, exist_ok=True)
    feature_file_path = os.path.join(feature_file_dir, "0.parquet")
    partition_id = 0
    bytes_written = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature_extractor", type=str, choices=["clip", "align"], help="Feature extractor")
