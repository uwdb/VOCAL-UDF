    ...
    for num_missing_udfs in 0 1 2; do
        for run_id in 0 1 2; do
            ...
            for query_id in {0..9}; do
                sbatch exp-evaluate_equivocal.sbatch $num_missing_udfs $query_id $run_id "charades" $query_filename
            done
        done
    done
done
    ...
    for num_missing_udfs in 0 1 2; do
        for run_id in 0 1 2; do
            ...
            for query_id in {0..14}; do
                sbatch exp-evaluate_equivocal.sbatch $num_missing_udfs $query_id $run_id "cityflow" $query_filename
            done
        done
    done
done
...
#         for run_id in 0; do
#             for query_id in {0..9}; do
#                 sbatch exp-evaluate_equivocal.sbatch $num_missing_udfs $query_id $run_id "gqa" $query_filename
#             done
#         done
#     done
# done
...
#                 sbatch exp-evaluate_equivocal.sbatch $num_missing_udfs $query_id $run_id "vaw" $query_filename
#             done
#         done
#     done
# done
    ...
    for num_missing_udfs in 0 1 2 3; do
        for run_id in 0 1 2; do
            for query_id in {0..29}; do
                sbatch exp-evaluate_equivocal.sbatch $num_missing_udfs $query_id $run_id "clevrer" $query_filename
            done
        done
    done
done