        ...
        for num_interpretations in 10; do
            ...
            for run in 0 1 2; do
                for query_id in {0..14}; do
                    sbatch exp-nl_to_dsl_cityflow_charades.sbatch $query_id $run "cityflow" $query_filename $budget $num_interpretations
                done
            done
        done
    done
        ...
        for num_interpretations in 10; do
            for run in 0 1 2; do
                for query_id in {0..9}; do
                    sbatch exp-nl_to_dsl_cityflow_charades.sbatch $query_id $run "charades" $query_filename $budget $num_interpretations
                done
            done
        done
    done
