...
def generate_vaw_queries(n_queries, min_npos, max_npos, npred, nattr_pred, nvars, nunsupported_pred, nunsupported_attr_pred, supported_list, unsupported_list, supported_attr_list, unsupported_attr_list, max_workers, dataset_name):
    """
    ...
    file_path = os.path.join(config['data_dir'], dataset_name, "unavailable_pred={}-unavailable_attr_pred={}-npred={}-nattr_pred={}-nvars={}-min_npos={}-max_npos={}.json".format(nunsupported_pred, nunsupported_attr_pred, npred, nattr_pred, nvars, int(min_npos), int(max_npos)))
    data = {"questions": queries}
    with open(file_path, "w") as file:
        json.dump(data, file, indent=4)

def generate_one_query(conn, min_npos, max_npos, npred, nattr_pred, nvars, nunsupported_pred, nunsupported_attr_pred, supported_list, unsupported_list, supported_attr_list, unsupported_attr_list, dataset_name):
    """
...
def generate_gt_labels_given_target_query(conn, query_str, min_npos, max_npos, new_modules):
    ...
    """
    input_vids = conn.execute("SELECT DISTINCT vid FROM vaw_objects order by vid asc").df()["vid"].tolist()
    print("Number of videos: {}".format(len(input_vids)))

    program = dsl_to_program(query_str)
    available_udf_names = ["black", "blue", "brown", "gray", "small", "metal", "long", "dark", "rounded", "orange", "white", "green", "large", "red", "wooden", "yellow", "tall", "silver", "standing", "round", "above", "beneath", "to_the_left_of", "to_the_right_of", "in_front_of", "behind"]
...
def init_table(conn, dataset):
    attribute_domain = ["black", "blue", "brown", "gray", "small", "metal", "long", "dark", "rounded", "orange", "white", "green", "large", "red", "wooden", "yellow", "tall", "silver", "standing", "round"]
...
def generate_vaw_queries_with_unsupported_udfs():
    ...
    ap.add_argument("--dataset_name", type=str, default="vaw", help="dataset name")
    ...
    if strategy is None:
        ...
        supported_attr_list = [
            ...
            {"predicate": "rounded", "parameter": None, "nargs": 1},
        ...
        unsupported_attr_list = [
            ...
            {"predicate": "wooden", "parameter": None, "nargs": 1},
            ...
            {"predicate": "round", "parameter": None, "nargs": 1},
        ]
    generate_vaw_queries(n_queries, min_npos, max_npos, npred, nattr_pred, nvars, nunsupported_pred, nunsupported_attr_pred, supported_list, unsupported_list, supported_attr_list, unsupported_attr_list, max_workers, dataset_name)
...
if __name__ == '__main__':
    generate_vaw_queries_with_unsupported_udfs()
    # generate_vaw_queries_udf_exclusion()
