...
class UDFProposer:
    ...
    def select_sample(self, udf_candidates_with_scores, udf_name, df_train, n_obj, labeled_index):
        ...
        unlabeled_index = np.setdiff1d(np.arange(len(df_train)), labeled_index, assume_unique=True)
        ...
        for udf_id, udf_candidate, _, _ in udf_candidates_with_scores:
            try:
                ...
                logger.debug("ERROR: failed to execute udf_candidate {}: {}".format(udf_id, e))
        ...
        logger.debug("constructing prediction matrix took {} seconds".format(time.time()-_start))
    ...
    def compute_udf_score(self, gt_udf, udf_candidate, udf_name, n_obj, df, df_newly_labeled=None):
        """
    ...
    def select(self, udf_signature, udf_description, gt_udf_name):
        ...
        if n_obj == 1:
            df_filtered = self.conn.execute(
                ...
                .format(self.n_train + self.n_test)).df()
            ...
            project_clause = ", ".join(["o1.{} as o1_{}".format(name, name) for name in names]) + ", " + ", ".join(["o2.{} as o2_{}".format(name, name) for name in names])
            df_filtered = self.conn.execute("""
                SELECT {}
        ...
        for iter in range(self.labeling_budget):
            ...
            new_labeled_index = self.select_sample(udf_candidates_with_scores, udf_name, df_train, n_obj, labeled_index)
            ...
            for _, udf_candidate, _, _ in udf_candidates_with_scores:
                updated_scores.append(self.compute_udf_score(gt_udf, udf_candidate, udf_name, n_obj, df_train.loc[labeled_index], df_train.loc[new_labeled_index]))
            ...
            logger.debug("updated udf_candidates_with_scores {}".format(["[udf_{}]: {}, {}".format(udf_id, score, loss_t) for udf_id, _, score, loss_t in udf_candidates_with_scores]))
            logger.debug("test segment_selection_time_per_iter time: {}".format(time.time() - _start_segment_selection_time_per_iter))
        segment_selection_time += time.time() - _start_segment_selection_time
        logger.debug("test segment_selection_time time: {}".format(segment_selection_time))
        ...
        for i in range(len(udf_candidates_with_scores)):
            f1_score_test = self.compute_udf_score(gt_udf, udf_candidates_with_scores[i][1], udf_name, n_obj, df_test)
        ...
        for best_candidate in best_candidates:
            f1_score_test = self.compute_udf_score(gt_udf, best_candidate[1], udf_name, n_obj, df_test)
...
class QueryExecutor:
    ...
    def run(self, program, y_true, debug=False):
        ...
        result, new_memo = exec_func(self.conn, program['query'], memo, self.inputs_table_name, input_vids, table_as_input_to_udf=True)
